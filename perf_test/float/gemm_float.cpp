...
int main(int argc, char *argv[]) {
    ...
    ChipSum::Common::Init(argc, argv);
    {
        ...
        for (int i=0; i<60; i++){
            ...
            float *A1 = static_cast<float *>(std::malloc(M*K * sizeof(float)));

            for(int i=0;i<M*K;++i)
            {
                A1[i] = float(rand()) / RAND_MAX;
            }
            Matrix A(M, K, A1);
            //A.Print();

            float *A2 = static_cast<float *>(std::malloc(K*N * sizeof(float)));

            for(int i=0;i<K*N;++i)
            {
                A2[i] = float(rand()) / RAND_MAX;
            }
            Matrix B(K, N, A2);
            //B.Print();

            float *A3 = static_cast<float *>(std::malloc(M*N * sizeof(float)));
